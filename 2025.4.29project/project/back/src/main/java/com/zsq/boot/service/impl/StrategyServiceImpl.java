...
public class StrategyServiceImpl implements StrategyService {
    ...
    public List<Strategy> generateStrategies(StrategyGenDTO dto) {
        log.info("开始生成策略，测站：{}，目标值：{}", dto.getStationName(), dto.getTargetValue());//示例：dto.getTargetValue()的值为5.0
    ...
    private Strategy createLowWaterStrategy(StrategyGenDTO dto) {
        ...
        strategy.setDescription(String.format(
            "针对%s测站水位降低（目标值%.1f）的调度策略，通过开启下游水闸降低水位",
            dto.getStationName(),
            dto.getTargetValue()
        ));
    ...
    private Strategy createHighWaterStrategy(StrategyGenDTO dto) {
        ...
        strategy.setStatus("推荐");
        strategy.setDescription(String.format(
            "针对%s测站水位（目标值%.1f）的调度策略，通过开启上游水闸提高水位",
            dto.getStationName(),
            dto.getTargetValue()
        ));
        strategy.setSimulating(false);
    ...
    private Strategy createBalanceStrategy(StrategyGenDTO dto) {
        ...
        strategy.setStatus("推荐");
        strategy.setDescription(String.format(
            "针对%s测站水位适中（目标值%.1f）的平衡策略，通过调节上下游水闸维持水位",
            dto.getStationName(),
            dto.getTargetValue()
        ));
        strategy.setSimulating(false);
    ...
    public Map<String, Object> simulateStrategy(Map<String, Object> strategy) {
        ...
        //Double targetValue = (Double)strategy.get("targetValue");//会报类型转换错误，可能被序列化为整型
        Number number = (Number) strategy.get("targetValue");
        Double targetValue = number.doubleValue();//目标水位值
        HydroData hydroData = waterLevelService.getByStationName(stationName);
        //在目标水位值上乘以一个随机数表示误差
        double randomFactor = ThreadLocalRandom.current().nextDouble(0.98, 1.02);//生成一个随机值，用于模拟预计水位
        Double predictionWaterLevel = targetValue*randomFactor;
        int randomNumTime = ThreadLocalRandom.current().nextInt(3, 6);  // 生成随机时间
